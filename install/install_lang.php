...
$i_message['install_lock'] = '您已安装过ttcms '.$_TSVERSION.'，如果需要重新安装，请先删除data目录下的install.lock文件';
$i_message['install_title'] = 'ttcms '.$_TSVERSION.' 安装向导';
$i_message['install_wizard'] = '安装向导';
$i_message['install_warning'] = '<strong>注意 </strong>这个安装程序仅仅用在你首次安装ttcms。如果你已经在使用 ttcms 或者要更新到一个新版本，请不要运行这个安装程序。';
$i_message['install_intro'] = '<h4>安装须知</h4><p>一、运行环境需求：PHP(5.3.12+)+MYSQL(5.5.12+)</p><p>二、安装步骤：<br /><br />1、使用ftp工具以二进制模式，将该软件包里的 ttcms 目录及其文件上传到您的空间，假设上传后目录仍旧为 ttcms。<br /><br />2、如果您使用的是Linux 或 Freebsd 服务器，先确认以下目录或文件属性为 (777) 可写模式。<br /><br />目录: data<br />目录: stroage/temp/<br />目录: install<br />目录: config<br />3、运行 http://yourwebsite/ttcms/install/install.php 安装程序，填入安装相关信息与资料，完成安装！<br />4、运行 http://yourwebsite/ttcms/cleancache.php 清除系统缓存文件！<br />5、运行 http://yourwebsite/ttcms/index.php 开始体验ttcms！</p>';
$i_message['install_start'] = '开始安装ttcms';
$i_message['install_license_title'] = '安装许可协议';
$i_message['install_license'] = '版权所有 (C) 2008-'.date('Y').'，ttcms 保留所有权利。

ttcms是由ttcms项目组独立开发的SNS程序，基于PHP脚本和MySQL数据库。本程序源码开放的，任何人都可以从互联网上免费下载，并可以在不违反本协议规定的前提下进行使用而无需缴纳程序使用费。
';
$i_message['install_agree'] = '我已看过并同意安装许可协议';
$i_message['install_disagree'] = '不同意';
$i_message['install_disagree_license'] = '您必须在同意授权协议的全部条件后，方可继续ttcms的安装';
...
$i_message['mysql_unsupport'] = '您的服务器不支持MYSQL数据库，无法安装ttcms。';
...
$i_message['install_mysql_prefix_intro'] = '同一数据库安装多个ttcms时可改变默认值';
...
$i_message['thinksns_rebuild'] = '数据库中已经安装过 ttcms，继续安装会清空原有数据！';
...
$i_message['install_success_intro'] = '<p>安装程序执行完毕，请尽快删除整个 install 目录，以免被他人恶意利用。如要重新安装，请删除data目录的 install.lock 文件！</p><p><a href="../index.php">请点击这里开始体验ttcms吧！</a></p>';
$i_message['install_dbFile_error'] = '数据库文件无法读取，请检查/install/ttcms.sql是否存在或者拥有读取权限。';
